...
OsmAnd::LatLon OsmAnd::CoordinateSearch::search(QString const &query)
{
    ...
    if (!url.isEmpty())
    {
        ...
        {
            auto parts = fragment.split(QRegularExpression{QLatin1String("/|,")});
        ...
        for (auto keyName : latLonSignleKeyNames)
        {
            ...
            {
                const auto reBeforeSecondComma = QRegularExpression{QLatin1String("[^,]*,[^,]*")};
                q = reBeforeSecondComma.match(part).captured(0);
                break;
            }
        }
    }
