    ...
    const auto ok = processArchive(
        ...
        (archive* /*archive*/, archive_entry* entry, bool& /*doStop*/, bool& /*match*/) -> bool
        {
            ...
            if (archive_entry_ctime_is_set(entry) != 0)
                item.creationTime = QDateTime::fromSecsSinceEpoch(static_cast<qint64>(archive_entry_ctime(entry)));
            if (archive_entry_mtime_is_set(entry) != 0)
                item.modificationTime = QDateTime::fromSecsSinceEpoch(static_cast<qint64>(archive_entry_mtime(entry)));
            if (archive_entry_atime_is_set(entry) != 0)
                item.accessTime = QDateTime::fromSecsSinceEpoch(static_cast<qint64>(archive_entry_atime(entry)));
