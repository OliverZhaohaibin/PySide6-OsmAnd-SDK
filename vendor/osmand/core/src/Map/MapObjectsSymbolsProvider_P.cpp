    ...
    const std::shared_ptr<const IQueryController>& queryController) const
{
    ...
    QHash< QString, QList < std::shared_ptr<CombinedPath> > > combinedPathsMap;
    for (const auto& mapObjectSymbolsGroup : rangeOf(constOf(symbolsGroups)))
    {
        ...
        const std::shared_ptr<CombinedPath> combinedPath(new CombinedPath(bbox31, mapObject, divisor31ToPixels));
        ...
        {
            QList< std::shared_ptr<CombinedPath> > combinedSymbolPathList;
    ...
    for (auto& pathsToCombine : combinedPathsMap)
    {
        ...
        for (auto attachedPaths = 0; attachedPaths < MAX_PATHS_TO_ATTACH && iterateFromStart; attachedPaths++)
        {
            ...
            for (auto pathIndex = 0; pathIndex < pathsToCombine.size() && !iterateFromStart; pathIndex++)
            {
                ...
                std::shared_ptr<CombinedPath> mainPath;
                std::shared_ptr<CombinedPath> pathToAttach;
...
OsmAnd::MapObjectsSymbolsProvider_P::CombinedPath::CombinedPath(
    const AreaI& bbox31,
...
QVector<OsmAnd::PointI> OsmAnd::MapObjectsSymbolsProvider_P::CombinedPath::getPoints() const
{
    return _points;
}

bool OsmAnd::MapObjectsSymbolsProvider_P::CombinedPath::isCombined() const
{
    return _combined;
}

bool OsmAnd::MapObjectsSymbolsProvider_P::CombinedPath::isAttachedToAnotherPath() const
{
    return _attachedToAnotherPath;
}

bool OsmAnd::MapObjectsSymbolsProvider_P::CombinedPath::isAttachAllowed(
    const std::shared_ptr<const CombinedPath>& other,
...
void OsmAnd::MapObjectsSymbolsProvider_P::CombinedPath::attachPath(const std::shared_ptr<CombinedPath>& other)
{
...
float OsmAnd::MapObjectsSymbolsProvider_P::CombinedPath::getLengthInPixels() const
{
