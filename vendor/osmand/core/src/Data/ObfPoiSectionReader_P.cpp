...
void OsmAnd::ObfPoiSectionReader_P::readTagGroups(const ObfReader_P& reader, QHash<uint32_t, QList<QPair<QString, QString>>> & tagGroups)
{
    ...
    for (;;)
    {
        ...
        switch (gpb::internal::WireFormatLite::GetTagFieldNumber(tag))
        {
            case 0:
                return;
            case OBF::OsmAndPoiTagGroups::kGroupsFieldNumber:
            {
                gpb::uint32 length;
                cis->ReadVarint32(&length);
                const auto oldLimit = cis->PushLimit(length);
    ...
    const std::shared_ptr<const IQueryController>& queryController)
{
    ...
    const auto matchesCategoriesFilter = [&categories, categoriesFilter]() -> bool
    {
        if (!categoriesFilter)
            return true;
        for (const auto categoryId : constOf(categories))
        {
        ...
        switch (tag)
        {
            case 0:
            {
                ...
                if (autogenerateId)
                    ...
                    amenity->id = id;
                amenity->values = detachedOf(intValues);
                for (auto it = stringOrDataValues.cbegin(); it != stringOrDataValues.cend(); ++it)
                    amenity->values.insert(it.key(), it.value());
