...
std::shared_ptr<OsmAnd::MapMarker::SymbolsGroup> OsmAnd::MapMarker_P::inflateSymbolsGroup(int subsection) const
{
    ...
    if (owner->pinIcon)
    {
        ...
        pinIconSymbol->content = QString::asprintf(
    ...
    int surfOrder = order;
    auto onMapSurfaceKeys = owner->onMapSurfaceIcons.keys();
    std::sort(onMapSurfaceKeys.begin(), onMapSurfaceKeys.end(),
    ...
    for (const auto& key : constOf(onMapSurfaceKeys))
    {
        const auto onMapSurfaceIcon = owner->onMapSurfaceIcons.value(key);
        if (!onMapSurfaceIcon)
            continue;
        ...
        onMapSurfaceIconSymbol->content = QString::asprintf(
