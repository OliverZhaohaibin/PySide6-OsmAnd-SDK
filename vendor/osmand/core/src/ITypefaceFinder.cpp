...
std::shared_ptr<OsmAnd::ITypefaceFinder::Typeface> OsmAnd::ITypefaceFinder::Typeface::fromData(const QByteArray& data, int faceIndex)
{
    if (data.isEmpty())
    {
        return nullptr;
    ...
    const auto skTypeface = SkiaUtilities::createTypefaceFromData(data, faceIndex);
    if (!skTypeface)
    {
        LogPrintf(LogSeverityLevel::Error,
    ...
    const auto hbFace = HarfbuzzUtilities::createFaceFromData(data, faceIndex);
    if (!hbFace)
    {
        LogPrintf(LogSeverityLevel::Error,
