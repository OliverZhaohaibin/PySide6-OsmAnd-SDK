...
bool OsmAnd::GPUAPI_OpenGLES2plus::initialize()
{
    ...
    GL_CHECK_RESULT;
    const auto glVersionMatch = QRegularExpression(QStringLiteral("(\\d+)\\.(\\d+)"))
        .match(QString(QLatin1String(reinterpret_cast<const char*>(glVersionString))));
    _glVersion = glVersionMatch.captured(1).toUInt() * 10 + glVersionMatch.captured(2).toUInt();
    ...
    GL_CHECK_RESULT;
    const auto glslVersionMatch = QRegularExpression(QStringLiteral("(\\d+)\\.(\\d+)"))
        .match(QString(QLatin1String(reinterpret_cast<const char*>(glslVersionString))));
    _glslVersion = glslVersionMatch.captured(1).toUInt() * 100;
    const auto minorVersion = glslVersionMatch.captured(2).toUInt();
    if (minorVersion < 10)
    ...
    GL_CHECK_RESULT;
    _extensions = extensionsString.split(QRegularExpression(QStringLiteral("\\s+")), Qt::SkipEmptyParts);
