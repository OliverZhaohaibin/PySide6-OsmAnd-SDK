    ...
    const int maxLines /*= 0*/)
{
    ...
    int linesCount = 0;
    const qsizetype count = lineStartIndices.size();
    for (qsizetype idx = 1; idx < count; ++idx)
    {
        auto startIndex = lineStartIndices[idx];
        result.push_back(QStringRef(&input, lastStartIndex, startIndex - lastStartIndex));
    ...
    if (maxLines <= 0 || linesCount < maxLines)
        result.push_back(QStringRef(&input, lastStartIndex, input.size() - lastStartIndex));
                                                  ...
                                                  bool checkBeginning, bool checkSpaces, bool equals)
{
    ...
    else
    {
        ...
        // this is not correct without (lowercaseAndAlignChars) because of Auhofstrasse != AuhofstraÃŸe
