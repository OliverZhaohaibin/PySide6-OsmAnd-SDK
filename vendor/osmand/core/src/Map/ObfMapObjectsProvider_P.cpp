    ...
    ObfMapObjectsProvider_Metrics::Metric_obtainData* const metric)
{
    ...
    // Obtain only the OBF data categories required by the current provider mode.
    ObfDataTypesMask requiredDataTypes;
    if (owner->mode == ObfMapObjectsProvider::Mode::OnlyBinaryMapObjects)
        requiredDataTypes.set(ObfDataType::Map);
    else if (owner->mode == ObfMapObjectsProvider::Mode::OnlyRoads)
        requiredDataTypes.set(ObfDataType::Routing);
    else
        requiredDataTypes.set(ObfDataType::Map).set(ObfDataType::Routing);
