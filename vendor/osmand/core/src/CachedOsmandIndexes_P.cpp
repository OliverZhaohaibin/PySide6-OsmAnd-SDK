...
#ifdef _MSC_VER
#include <io.h>
#define OSMAND_OPEN _open
#else
#define OSMAND_OPEN open
#endif
...
{
    int fileDescriptor = OSMAND_OPEN(filePath.toStdString().c_str(), O_RDONLY);
    if (fileDescriptor < 0)
    {
...
void OsmAnd::CachedOsmandIndexes_P::writeToFile(const QString& filePath)
{
    ...
    {
        int fileDescriptor = OSMAND_OPEN(filePath.toStdString().c_str(), O_RDWR);
        if (fileDescriptor < 0) {
