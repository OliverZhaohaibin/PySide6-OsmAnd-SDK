...
OsmAnd::ResourcesManager::ResourceType OsmAnd::ResourcesManager_P::getIndexType(const QStringView resourceTypeValue)
{
    auto resourceType = ResourceType::Unknown;
    ...
    while (!xmlReader.atEnd() && !xmlReader.hasError())
    {
        ...
        if (resourceType == OsmAnd::ResourcesManager::ResourceType::Unknown)
        {
            LogPrintf(LogSeverityLevel::Verbose,
                      ...
                      qPrintable(resourceTypeValue.toString()),
                      qPrintable(name));
            continue;
        }
        ...
        if (!ok)
        {
            LogPrintf(LogSeverityLevel::Warning,
                ...
                qPrintable(timestampValue.toString()),
                qPrintable(name));
            continue;
        }
        ...
        if (!ok)
        {
            LogPrintf(LogSeverityLevel::Warning,
                ...
                qPrintable(containerSizeValue.toString()),
                qPrintable(name));
            continue;
        }
        ...
        if (!ok)
        {
            LogPrintf(LogSeverityLevel::Warning,
                ...
                qPrintable(contentSizeValue.toString()),
                qPrintable(name));
            continue;
        }
