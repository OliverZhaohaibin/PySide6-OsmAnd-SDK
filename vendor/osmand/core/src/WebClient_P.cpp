    ...
    if (!rangeHeaderSupported)
    {
        ...
        if (rangeNetworkReply->error() == QNetworkReply::NoError)
        {
            ...
            if (networkReply->hasRawHeader("Content-Range"))
            {
                ...
                const auto contentRangeHeaderParser = QRegularExpression(QLatin1String("bytes (\\d+)-(\\d+)\\/.*"));
                const auto contentRangeMatch = contentRangeHeaderParser.match(contentRangeHeader);
                if (contentRangeMatch.hasMatch())
                    contentSize = contentRangeMatch.captured(2).toULongLong(&rangeHeaderSupported);
            }
        }
