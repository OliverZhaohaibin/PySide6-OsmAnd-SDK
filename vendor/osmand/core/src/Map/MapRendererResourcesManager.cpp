    ...
    const int zoomLevelOffset)
{
    ...
    if (isMapLayer || isElevationData || isMap3DObjects)
    {
        for (const auto& activeTileId : constOf(activeTiles))
        {
            if (!isElevationData && !visibleTiles.contains(activeTileId))
                continue;
            if (activeZoom < minZoom && !isElevationData && !isMap3DObjects)
            {
        ...
        for (const auto& resourcesCollection : constOf(resourcesCollections))
        {
                    ...
                    std::dynamic_pointer_cast<MapRendererKeyedResourcesCollection::Snapshot>(collectionSnapshot))
            {
                ...
                if (obtainProviderFor(resourcesCollection.get(), provider_))
                {
                    ...
                    const auto collectionKeys = keyedResourcesCollection->getKeys();
                    const auto providerKeys = provider->getProvidedDataKeys();
                    QSet<IMapKeyedDataProvider::Key> collectionKeysSet;
                    for (const auto key : constOf(collectionKeys))
                        collectionKeysSet.insert(key);
                    QSet<IMapKeyedDataProvider::Key> providerKeysSet;
                    for (const auto key : constOf(providerKeys))
                        providerKeysSet.insert(key);

                    if (collectionKeysSet != providerKeysSet)
                        updatesPresent = true;
                }
            }
    ...
    QMap<ZoomLevel, QSet<TileId>>& visibleTiles, QSet<TileId>& extraDetailedTiles, int zoomLevelOffset)
{
    ...
    for (const auto& resourcesCollection : constOf(resourcesCollections))
    {
        ...
        if (dataProvider != nullptr && std::dynamic_pointer_cast<IMapRendererKeyedResourcesCollection>(resourcesCollection))
        {
            ...
            const auto providedKeys = keyedDataProvider->getProvidedDataKeys();
            QSet<IMapKeyedDataProvider::Key> providedKeysSet;
            for (const auto key : constOf(providedKeys))
                providedKeysSet.insert(key);
        ...
        if (const auto tiledResourcesCollection = std::dynamic_pointer_cast<IMapRendererTiledResourcesCollection>(resourcesCollection))
        {
            ...
            const bool isMapLayer = resourcesType == MapRendererResourceType::MapLayer;
            const bool isElevationData = resourcesType == MapRendererResourceType::ElevationData;
            ...
            {
                assert(checkVisible);
                addCombinedTilesToAvoidCleanup(tiledResourcesCollection, tiles);
            }
            ...
            if (tiledProvider && (isMapLayer || isElevationData || isMap3DObjects))
            {
                minZoom = Utilities::clipZoomLevel(tiledProvider->getMinZoom());
                maxZoom = Utilities::clipZoomLevel(tiledProvider->getMaxZoom());
            ...
            for (const auto& tilesEntry : rangeOf(constOf(tiles)))
            {
                ...
                const auto neededZoom = isElevationData || zoomLevelOffset == 0 ? activeZoom
                    : static_cast<ZoomLevel>(std::min(activeZoom
                        + std::min(zoomLevelOffset, maxMissingDataUnderZoomShift), static_cast<int>(maxZoom)));
                const auto extraZoom =
                    isElevationData || zoomLevelOffset != 0 || activeZoom != currentZoom || extraDetailedTiles.empty()
                    ? neededZoom
                    : static_cast<ZoomLevel>(std::min(activeZoom
                ...
                const int levelCount = 1 + (isSymbols ? 0 : maxMissingDataUnderZoomShift);
                for (const auto& activeTileId : constOf(activeTiles))
                {
                    ...
                    if (isMapLayer || isElevationData || isMap3DObjects)
                    {
                        ...
                        for (int absZoomShift = 1; absZoomShift <= MaxZoomLevel; absZoomShift++)
                        {
                            ...
                            if (Q_LIKELY(!debugSettings->rasterLayersUnderscaleForbidden))
                            {
                                ...
                                if (underscaledZoom <= maxZoom && absZoomShift <= maxMissingDataUnderZoomShift)
                                {
                                    const auto underscaledTileIdsN = Utilities::getTileIdsUnderscaledByZoomShift(
                                        activeTileId,
                                        absZoomShift);
